...
- After a successful save, clear the input field and finish the activity, so the same text can never be written twice. `android:noHistory` does not finish the activity on screen-off, so without this a power-cycle resumed the surface with the already-saved text still in it and wrote a duplicate note on every subsequent screen-off.
- Keep the display awake while the text-entry surface is showing (`FLAG_KEEP_SCREEN_ON`, as `capture/TapToStopActivity` already does), so that a screen-off is always a deliberate power press and never an idle display timeout committing a half-written note.
