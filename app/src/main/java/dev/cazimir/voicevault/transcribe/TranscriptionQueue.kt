...
object TranscriptionQueue {
    ...
    suspend fun transcribe(context: Context, wavFile: File): TranscriptionOutcome =
        transcribe(
            wavFile,
            writeNote = { captureId, text -> VaultWriter.writeNote(context, text, captureId) },
            deleteDiagnostic = { captureId -> VaultWriter.deleteDiagnostic(context, captureId) },
        ) { loadWhisper(context) }

    /**
     * Test seam: [loadTranscriber] replaces the real whisper-android load so outcome
     * classification can be exercised without an Android [Context], [writeNote] replaces the
     * SAF note write so a forced failure never touches real storage, and [deleteDiagnostic]
     * replaces the post-success diagnostic cleanup.
     */
    internal suspend fun transcribe(
        wavFile: File,
        writeNote: (captureId: String, text: String) -> Unit = { _, _ -> error("writeNote not provided") },
        deleteDiagnostic: (captureId: String) -> Unit = {},
        ...
        return try {
            mutex.withLock {
                ...
                // which must stay recoverable - the audio then waits in the pending folder and is
                ...
                processOne(loaded, wavFile, writeNote, deleteDiagnostic)
            }
        } finally {
    ...
    private suspend fun processOne(
        ...
        wavFile: File,
        writeNote: (captureId: String, text: String) -> Unit,
        deleteDiagnostic: (captureId: String) -> Unit
    ): TranscriptionOutcome {
        val text = try {
        ...
        try {
            writeNote(captureId, text + "\n")
        } catch (t: Throwable) {
            Log.w(Config.LOG_TAG, "note write failed for $captureId", t)
            return TranscriptionOutcome.NoteWriteFailed(t)
        }
        // Still inside the queue mutex: the audio is gone before any other entry can pick it up.
        wavFile.delete()
        deleteDiagnostic(captureId)
        return TranscriptionOutcome.Success(captureId)
    }
